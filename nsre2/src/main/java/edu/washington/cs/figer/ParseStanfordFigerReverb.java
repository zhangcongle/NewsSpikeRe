...
public class ParseStanfordFigerReverb {
	...
	public static List<Pair<Integer, Integer>> getAllNamedEntityMentions(
			CoreMap sentence) {
	...
	public static void callreverb(ReVerbExtractorWrap rew,
			BinaryExtractionNormalizer normalizer,
			HeadNounExtractor headnoun_extractor,
			FigerParsedSentence ps) {
		try {
...
//				pa.numSentence = sentences.size();
				for (CoreMap sentence : annotation.get(SentencesAnnotation.class)) {
					FigerParsedSentence ps = new FigerParsedSentence(lineId, sentId, sentence,
							Preprocessing.gsf);
//					pa.parsedsentence.add(ps);
					...
					for (Pair<Integer, Integer> offset : entityAllMentionOffsets) {
						...
						if (validTags.contains(ner.slabel)) {
							ner.flabel = sys.predict(annotation, sentId,
									offset.first, offset.second);
						}
...
//		String text = "UPDATE January 29 , 2015 8:39 EST : Ladd was executed Thursday night by lethal injection in Huntsville , Texas .";
...
//		int i = 0;
//		for (CoreMap sentence : annotation.get(SentencesAnnotation.class)) {
//			FigerParsedSentence ps = new ParsedSentence(pa.sectionId, i++, sentence, Preprocessing.gsf);
//			pa.parsedsentence.add(ps);
...
//		CorenlpNews cn = new CorenlpNews();
//		cn.parsedarticle = pa;
//		boolean success = callreverb(pa, cn, rew, normalizer, headnoun_extractor);
//		// System.out.println(gson.toJson(pa));
//		System.out.println(gson.toJson(cn));
//	}
