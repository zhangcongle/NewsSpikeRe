...
public class WebDemoServlet extends HttpServlet {
	...
	private final static Logger logger = LoggerFactory.getLogger(WebDemoServlet.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int sentId = 0;
			response.getWriter()
					.write("<html>\n<head>" + "<link rel=\"stylesheet\" " + "href=\"https://maxcdn.bootstrapcdn.com"
							+ "/bootstrap/3.3.1/css/bootstrap.min.css\">" + "</head>\n<body>\n");
			for (CoreMap sentence : annotation.get(SentencesAnnotation.class)) {
				...
				for (CoreLabel label : sentence.get(TokensAnnotation.class)) {
					...
					words.add(new StringBuilder("<td>" + label.originalText() + "&nbsp;</td>"));
				}

				// System.out.println("[s" + sentId + "]"
				// + sentence.get(TextAnnotation.class));
				List<Pair<Integer, Integer>> entityMentionOffsets = FigerSystem.getNamedEntityMentions(sentence);
				for (Pair<Integer, Integer> offset : entityMentionOffsets) {
					...
					words.get(offset.first).append(mention.toString().replace("</td><td>", "")
							.replace("<td>", "<td><p class=\"bg-success\">").replace("</td>", "</p></td>"));
					String label = figer.predict(annotation, sentId, offset.first, offset.second);
					...
					tags.get(offset.first)
							.append("<td><p class=\"bg-info\">" + label.replace(",", "<br/>") + "</p></td>");
				}
				response.getWriter().write("<tr><td><p class=\"text-primary\">Sentence:</p></td>");
				for (int i = 0; i < tags.size(); i++) {
					response.getWriter().write(words.get(i).toString());
				}
				response.getWriter().write("</tr>\n");
				response.getWriter().write("<tr><td><p class=\"text-info\">Annotation:</p></td>");
				for (int i = 0; i < tags.size(); i++) {
